...
void move() {
  ...
  if((keyCode==36)&&(globglogabgalab>1)){
  globglogabgalab-=1;
  }
  frameRate(globglogabgalab);
  println(keyCode);
...
void direction() {
 if(direction.equals("up")||direction.equals("right")||direction.equals("left")){
  if(keyPressed){        
   if(keyCode==38){
     direction="up";
   }
 ...
 if(keyCode==37){
   direction="left";
     ...
     if(direction.equals("up")||direction.equals("right")||direction.equals("down")){
  if(keyPressed){        
 if(keyCode==39){
   direction="right";
   }
...
checkTailCollision();
  ...
  Segment headImposter = new Segment(head.getX(),head.getY());
  tail.add(headImposter);
